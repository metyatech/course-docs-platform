...
test('question-spec markdown transforms into <Exercise> + <Solution>', async () => {
  const { default: remarkQuestionSpecToExercise } = await import(
    pluginModulePath
  );
    ...
    children: [
      {
        type: 'heading',
        depth: 1,
        children: [{ type: 'text', value: '問題1（テスト）' }],
      },
      {
        type: 'heading',
        depth: 2,
      ...
      {
        type: 'paragraph',
        children: [{ type: 'text', value: 'ここが問題文です。' }],
      },
      {
        type: 'heading',
        depth: 2,
        children: [{ type: 'text', value: 'Scoring' }],
      },
      {
        type: 'paragraph',
        children: [{ type: 'text', value: '3: 期待通り動作する' }],
      },
      {
        type: 'heading',
        depth: 2,
      ...
      {
        type: 'paragraph',
        children: [{ type: 'text', value: 'ここが解説です。' }],
      },
    ],
  };
    ...
    children: [
      {
        type: 'heading',
        depth: 1,
        children: [{ type: 'text', value: '問題（穴埋め）' }],
      },
      {
        type: 'heading',
        depth: 2,
    ...
    children: [
      {
        type: 'heading',
        depth: 1,
        children: [{ type: 'text', value: '問題（コード）' }],
      },
      {
        type: 'heading',
        depth: 2,
    ...
    children: [
      {
        type: 'heading',
        depth: 1,
        children: [{ type: 'text', value: '問題（見出し）' }],
      },
      {
        type: 'heading',
        depth: 2,
      ...
      },
      {
        type: 'heading',
        depth: 3,
        children: [{ type: 'text', value: '解説' }],
      },
      {
        type: 'paragraph',
        children: [{ type: 'text', value: 'prompt content' }],
      },
      {
        type: 'heading',
        depth: 2,
      ...
      {
        type: 'paragraph',
        children: [{ type: 'text', value: 'explanation content' }],
      },
    ],
  };
...
test('### Exam under Prompt becomes a tip admonition', async () => {
    ...
    children: [
      {
        type: 'heading',
        depth: 1,
        children: [{ type: 'text', value: '問題（Exam）' }],
      },
      {
        type: 'heading',
        depth: 2,
  ...
  const admonitions = (exercise.children ?? []).filter(
    (child) =>
      child?.type === 'mdxJsxFlowElement' && child?.name === 'Admonition'
  );

  assert.equal(admonitions.length, 1);
  const tip = admonitions[0];
  const title = (tip.attributes ?? []).find(
    (attr) => attr?.name === 'title'
  )?.value;
  const type = (tip.attributes ?? []).find(
    (attr) => attr?.name === 'type'
  )?.value;
    ...
    children: [
      {
        type: 'heading',
        depth: 1,
        children: [{ type: 'text', value: 'Not a question spec' }],
      },
      {
        type: 'heading',
        depth: 2,
      ...
      },
      {
        type: 'paragraph',
        children: [
          { type: 'text', value: 'Looks similar but should not transform.' },
        ],
      },
    ],
  };
